html_parser <- function(link,dir){
  ...
  files<-files[files!= "atp_matches_doubles_yyyy.csv"]
  file.path(dir, files)
}


game_checker <- function(x){
...
tidy_game <- function(x){
  d<-game_checker(x)
  if (all(d)){
    return(x)
  } else {
    z<-unlist(x) %>% toString() %>% str_extract_all("[0-9]")
    z<-z[[1]]
    if ((length(z)&&2)!=0){
      return(list(NA))
    } else{
      return(split(z,ceiling((1:length(z))/2)))
    }
  }
}


tidy_match_data_patched <- function(data, atp = T){
  
  # Parse scores 
  games_won <- sapply(strsplit(data$score, split = " "), function(x){
    t<-gsub("[:;&.]","",gsub("\\([0-9]+\\)","",gsub("[A-Za-z]","",x)))
    t <- gsub("[?)(>!]","",t)
    strsplit(t[t!=""], split="-")
  }
  ) %>% sapply(function(x) tidy_game(x))
